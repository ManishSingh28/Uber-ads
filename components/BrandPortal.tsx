...
import { Home, LayoutGrid, FileText, UserCircle, Library, Truck, Settings, HelpCircle, Plus, X } from 'lucide-react';
...
export default function BrandPortal({ setActivePage }: BrandPortalProps) {
  ...
  useEffect(() => {
    let chartInstance: Chart | null = null;
    // Only render chart if modal is open AND it's not a draft campaign
    if (isModalOpen && selectedCampaign?.status !== 'Draft' && modalChartRef.current) {
  ...
  // Helper component to render draft fields
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)] bg-gray-50">
                                  ...
                                  <tr key={camp.id} onClick={() => {setSelectedCampaign(camp); setIsModalOpen(true)}} className="hover:bg-blue-50/50 cursor-pointer transition-colors group">
                                      ...
                                      <td className="p-6">
                                        <span className={`text-[9px] font-black px-3 py-1.5 rounded-full uppercase 
                                            ${camp.status === 'Live' ? 'bg-green-100 text-green-700' : 
                                            camp.status === 'Draft' ? 'bg-amber-100 text-amber-700 border border-amber-200' : 
                ...
                {selectedCampaign.status === 'Draft' ? (
                    ...
                    <div className="flex flex-col flex-1 overflow-hidden bg-gray-50">
                        ...
                        <div className="bg-white border-t border-gray-200 p-6 flex justify-between items-center">
                            <button onClick={() => setIsModalOpen(false)} className="bg-white border border-gray-200 text-gray-700 px-8 py-4 rounded-xl font-bold hover:bg-gray-50 hover:text-black transition-all uppercase text-sm tracking-wide shadow-sm">
                                ← Back
                            </button>
                            <div className="flex gap-4">
                                <button onClick={() => { alert('Draft Discarded!'); setIsModalOpen(false); }} className="bg-white border border-gray-200 text-gray-700 px-6 py-4 rounded-xl font-bold hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all uppercase text-sm tracking-wide shadow-sm">
                                    Discard Campaign
                                </button>
                                <button onClick={() => { setActivePage('create-campaign'); setIsModalOpen(false); }} className="bg-blue-600 border border-blue-600 text-white px-10 py-4 rounded-xl font-black hover:bg-blue-700 transition-all shadow-lg hover:shadow-blue-200 uppercase text-sm tracking-wide active:scale-95">
                                    Review & Submit →
                                </button>
                            </div>
                        </div>
      ...
      {/* HEATMAP MODAL REMAINS UNCHANGED */}
      {isHeatmapOpen && (
