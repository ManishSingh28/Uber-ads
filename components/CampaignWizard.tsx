// components/CampaignWizard.tsx
"use client";
import React, { useState } from 'react';
import Step1Details  from './wizard/Step1Details';
import Step2Location from './wizard/Step2Location';
import Step3Review   from './wizard/Step3Review';
import { ArrowRight, CheckCircle } from 'lucide-react';

interface CampaignWizardProps {
  setActivePage: (page: string) => void;
}

export default function CampaignWizard({ setActivePage }: CampaignWizardProps) {
  const [currentStep, setCurrentStep] = useState(1);
  const [submitted, setSubmitted] = useState<'approval' | 'draft' | null>(null);

  const [formData, setFormData] = useState({
    name: '', brand: '', category: '', costModel: '', budget: 0,
    startDate: '', endDate: '',
    cities: [] as any[],
    artwork: null as string | null,
    artworkPreview: null as string | null,
    artworkName: null as string | null,
  });

  const updateForm = (newData: Partial<typeof formData>) =>
    setFormData(prev => ({ ...prev, ...newData }));

  const today = new Date().toISOString().split('T')[0];

  const isStep1Valid =
    !!formData.name?.trim() &&
    !!formData.brand &&
    !!formData.category &&
    !!formData.costModel &&
    formData.budget >= 15000 &&
    !!formData.startDate && formData.startDate >= today &&
    !!formData.endDate && formData.endDate > formData.startDate;

  const isStep2Valid = (formData.cities?.length ?? 0) > 0;

  const isFullyValid = isStep1Valid && isStep2Valid && !!formData.artwork;
  const isDraft      = isStep1Valid && isStep2Valid && !formData.artwork;

  const handleGoBack = () => {
    if (currentStep > 1) setCurrentStep(currentStep - 1);
    else setActivePage('brand-portal');
  };

  const handleDiscard = () => {
    if (confirm('Discard this campaign? All progress will be lost.')) setActivePage('brand-portal');
  };

  const handleSaveDraft = () => {
    setSubmitted('draft');
  };

  const handleContinue = () => {
    if (currentStep < 3) { setCurrentStep(currentStep + 1); return; }
    // Step 3 — submit or save draft
    if (isFullyValid) setSubmitted('approval');
    else setSubmitted('draft');
  };

  // ── Success screen ──
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto p-6 pt-24 text-center">
        <div className={`w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6 ${submitted === 'approval' ? 'bg-green-100' : 'bg-amber-100'}`}>
          <CheckCircle size={36} className={submitted === 'approval' ? 'text-green-600' : 'text-amber-600'} />
        </div>
        <h2 className="text-3xl font-bold tracking-tighter mb-3">
          {submitted === 'approval' ? 'Submitted for Approval' : 'Saved as Draft'}
        </h2>
        <p className="text-zinc-500 text-sm leading-relaxed mb-8 max-w-sm mx-auto">
          {submitted === 'approval'
            ? 'Your campaign has been submitted. The Uber-Ads team will review it within 1–2 business days and move it to Ops Queue once approved.'
            : 'Campaign saved as draft. Complete the missing artwork upload and resubmit when ready.'
          }
        </p>
        {submitted === 'draft' && (
          <div className="bg-amber-50 border border-amber-100 rounded-xl p-4 text-left mb-6">
            <p className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-2">What's missing</p>
            <p className="text-xs font-bold text-amber-800">Creative Artwork — Upload a Vector (SVG) or UHD raster (PNG/JPG, min 2000×2000px) to enable submission.</p>
          </div>
        )}
        <div className="flex gap-3">
          <button
            onClick={() => setActivePage('brand-portal')}
            className="flex-1 bg-black text-white py-4 rounded-xl font-bold hover:bg-zinc-800 transition-all"
          >
            Back to Dashboard
          </button>
          {submitted === 'draft' && (
            <button
              onClick={() => { setSubmitted(null); setCurrentStep(2); }}
              className="flex-1 bg-zinc-100 text-zinc-900 py-4 rounded-xl font-bold hover:bg-zinc-200 transition-all"
            >
              Upload Artwork
            </button>
          )}
        </div>
      </div>
    );
  }

  const canContinue =
    currentStep === 1 ? isStep1Valid :
    currentStep === 2 ? isStep2Valid :
    true; // Step 3 always allows continue (submit or draft)

  const continueLabel =
    currentStep < 3 ? 'Continue →' :
    isFullyValid     ? 'Submit for Approval' :
    'Save as Draft';

  return (
    <div className="max-w-5xl mx-auto p-6 pt-10 pb-24">
      {/* Header */}
      <div className="flex items-center mb-10">
        <button onClick={() => setActivePage('brand-portal')} className="mr-4 text-gray-500 hover:text-black transition-colors font-bold flex items-center gap-2">
          <ArrowRight size={16} className="rotate-180" /> Back to Portal
        </button>
      </div>
      <h2 className="text-4xl font-black uppercase mb-10">New Campaign</h2>

      {/* Steps */}
      <div className="flex justify-between mb-12 relative max-w-2xl mx-auto">
        <div className="absolute left-0 right-0 top-1/2 h-0.5 bg-gray-200 -z-10" />
        {[1, 2, 3].map(step => (
          <div key={step} className="flex flex-col items-center">
            <div className={`w-12 h-12 rounded-full font-black flex items-center justify-center border-4 border-gray-50 shadow-sm transition-colors ${currentStep >= step ? 'bg-black text-white' : 'bg-gray-200 text-gray-400'}`}>
              {step}
            </div>
            <span className={`text-[10px] font-black mt-3 uppercase tracking-wider ${currentStep >= step ? 'text-black' : 'text-gray-400'}`}>
              {step === 1 ? 'Campaign Details' : step === 2 ? 'Banner & Locations' : 'Review & Pay'}
            </span>
          </div>
        ))}
      </div>

      {/* Step content */}
      <div className="bg-white border border-gray-200 rounded-[32px] p-10 shadow-sm mb-8 min-h-[500px]">
        {currentStep === 1 && <Step1Details   formData={formData} updateForm={updateForm} />}
        {currentStep === 2 && <Step2Location  formData={formData} updateForm={updateForm} />}
        {currentStep === 3 && <Step3Review    formData={formData} />}
      </div>

      {/* Footer */}
      <div className="flex justify-between items-center px-2">
        <button
          onClick={handleGoBack}
          className="bg-white border border-gray-200 text-gray-700 px-8 py-4 rounded-xl font-bold hover:bg-gray-50 hover:text-black transition-all uppercase text-sm tracking-wide shadow-sm"
        >
          ← Go Back
        </button>
        <div className="flex gap-4 items-center">
          <button
            onClick={handleDiscard}
            className="bg-white border border-gray-200 text-gray-700 px-6 py-4 rounded-xl font-bold hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-all uppercase text-sm tracking-wide shadow-sm"
          >
            Discard
          </button>
          <button
            onClick={handleSaveDraft}
            className="bg-white border border-gray-200 text-gray-700 px-6 py-4 rounded-xl font-bold hover:bg-gray-50 hover:text-black transition-all uppercase text-sm tracking-wide shadow-sm"
          >
            Save Draft
          </button>
          <button
            onClick={handleContinue}
            disabled={!canContinue}
            className={`border px-10 py-4 rounded-xl font-black transition-all shadow-lg uppercase text-sm tracking-wide active:scale-95 disabled:opacity-40 disabled:cursor-not-allowed ${
              currentStep === 3 && isFullyValid
                ? 'bg-black border-black text-white hover:bg-zinc-800'
                : currentStep === 3 && !isFullyValid
                ? 'bg-amber-500 border-amber-500 text-white hover:bg-amber-600'
                : 'bg-blue-600 border-blue-600 text-white hover:bg-blue-700 hover:shadow-blue-200'
            }`}
          >
            {continueLabel}
          </button>
        </div>
      </div>
    </div>
  );
}